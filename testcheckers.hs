...
getPiece [] (char, int) = Empty(char, int)
getPiece (x:xs) (char, int) = (getPieceAux x (char, int)):(getPiece xs (char, int))

getPieceAux :: [Piece] -> (Char, Int) -> Piece
getPieceAux [] (char, int) = Empty
getPieceAux (x:xs) (char, int)
    | snd x == (char, int) = x
    | otherwise = x:(getPieceAux xs (char, int))



{-- finds the piece that is to me moved, true if there is a piece of that color placed in that position
validPiece :: Checkerboard -> (Char, Int) -> Color -> Bool
validPiece [] (char, int) color = False
validPiece (x:xs) (char, int) color = (validPieceAux x (char, int) color):(validPiece xs (char, int) color)


-- 
validPieceAux :: [Piece] -> (Char, Int) -> Color -> Bool
validPieceAux [] (char, int) color = False
validPieceAux (x:xs) (char, int) color
    | snd x == (char, int) = checkColor 
    | otherwise = x:(validPieceAux xs (char, int) color)

checkColor :: Piece -> Color -> Bool
checkColor piece color = False
checkColor (piece (char, int)) 
    | pieceColor == color = True
    | otherwise = False
    --}
